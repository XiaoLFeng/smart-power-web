      ...
      if (getRes.output === "Success") {
        ...
        if (this.getElectricity.electricity.length === 0) {
          ...
          this.getElectricity.electricity.forEach((item: ElectricityEntity) => {
            this.calculateElectricity += item.peak_electricity
            this.calculateElectricity += item.valley_electricity
          });
          // 统计总电费
          this.getElectricity.electricity.forEach((item: ElectricityEntity) => {
            this.calculateElectricityBill += item.peak_electricity_bill
            this.calculateElectricityBill += item.valley_electricity_bill
          });
        }
      }
    }
